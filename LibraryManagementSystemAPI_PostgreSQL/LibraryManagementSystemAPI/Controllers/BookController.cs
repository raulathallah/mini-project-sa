...
namespace LibraryManagementSystemAPI.Controllers
{
    ...
    public class BookController : ControllerBase
    {
        ...
        {
            BookListDto res = await _bookService.GetAllBook();
        ...
        {
            BookDetailDto res = await _bookService.GetBookById(id);
            if(!res.Status)
            {
                return NotFound(res);
            }
        ...
        [ProducesResponseType(typeof(BookDetailDto), StatusCodes.Status200OK)]
        [ProducesResponseType(typeof(BookDetailDto), StatusCodes.Status400BadRequest)]
        public async Task<IActionResult> AddNewBook([FromBody] BookAddDto value)
        {
            BookResponseDto res = await _bookService.AddNewBook(value);
            if (!res.Status)
            {
        ...
        [ProducesResponseType(typeof(BookDetailDto), StatusCodes.Status400BadRequest)]
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateBook(int id, [FromBody] BookAddDto value)
        {
            BookResponseDto res = await _bookService.UpdateBook(id, value);
            if(!res.Status)
            {
                return BadRequest(res);
            }
        ...
        {
            BookResponseDto res = await _bookService.DeleteBook(id);
            if (!res.Status)
            {
                return BadRequest(res);
            }
